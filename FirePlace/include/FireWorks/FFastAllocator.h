...
protected:
	...
	friend void* operator new( size_t uiSize, FFixedBlockAllocator< T, false, AllocPool, nSubID, BASE_ALLOC >& kAlloc );
};

// Placement new on a FFastAllocator allows allocation and construction to be combined.
template< class T, unsigned int AllocPool, unsigned int nSubID, class BASE_ALLOC >
void* operator new(size_t uiSize, FFixedBlockAllocator< T, false, AllocPool, nSubID, BASE_ALLOC >& kAlloc )
{
